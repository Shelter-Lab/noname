...
// 正是我们要的快路径;下次真换版又会有新的 install 重新开窗
// (这与"正在下载"那种必须持久化的状态不同 —— 那个丢了会误杀下载)。
//
// 【★ 更正(2026-08-13)】这里原来写着「丢了只是更快,没有正确性损失」——**那句是错的**。
// 对"改了内容但路径不变"的素材(重画的卡面/立绘),窗口丢了就是**永久读旧字节**:
// 本标记只对"窗口开着时恰好被请求到"的素材生效,而 iOS 回收 SW 很凶,
// 等玩家翻到那张牌时装它的那个实例早没了,于是那张图一辈子不会被校验。
// 实测就是这么发作的:换了 25 张卡面部署后设备上仍是旧图,点「下载离线资源」还说"已下载完毕"。
